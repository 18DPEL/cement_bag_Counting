...
# Frame dimensions
FRAME_WIDTH = 840  # Changed from 640 to 840
FRAME_HEIGHT = 480
...
class VideoThread(QThread):
    ...
    def __del__(self):
        """Destructor to ensure recordings are saved even if thread is destroyed unexpectedly"""
        try:
            if self.recording:
                self.stop_recording()
        except:
    ...
    def start_recording(self):
        ...
        if not self.recording:
            try:
                ...
                if self.primary_video_writer.isOpened():
                    self.recording = True
                    if self.secondary_video_source and self.secondary_video_writer and self.secondary_video_writer.isOpened():
                        self.recording_status_signal.emit(True, f"Recording started for both cameras: {timestamp}")
                    else:
                        self.recording_status_signal.emit(True, f"Recording started for primary camera: {timestamp}")
                else:
        ...
        if self.recording:
            try:
                ...
                if self.primary_video_writer is not None:
                    ...
                    self.primary_video_writer = None
                    if self.primary_recording_filename:
                        saved_files.append(f"Primary: {os.path.basename(self.primary_recording_filename)}")
                
                # Stop secondary recording
                if self.secondary_video_writer is not None:
                    self.secondary_video_writer.release()
                    self.secondary_video_writer = None
                    if self.secondary_recording_filename:
                        saved_files.append(f"Secondary: {os.path.basename(self.secondary_recording_filename)}")
                
                self.recording = False
                if saved_files:
                    self.recording_status_signal.emit(False, f"Recordings saved: {', '.join(saved_files)}")
                else:
                    self.recording_status_signal.emit(False, "Recording stopped")
                
            except Exception as e:
                self.recording_status_signal.emit(False, f"Error stopping recording: {e}")
    
    def write_frame_to_recording(self, primary_frame, secondary_frame=None):
        """Write annotated frames to recordings if recording is active"""
        if self.recording:
            # Write primary camera frame
            if self.primary_video_writer is not None:
                try:
                    self.primary_video_writer.write(primary_frame)
                except Exception as e:
                    print(f"Error writing primary frame to recording: {e}")
            
            # Write secondary camera frame if available
            if secondary_frame is not None and self.secondary_video_writer is not None:
                try:
                    self.secondary_video_writer.write(secondary_frame)
                except Exception as e:
                    print(f"Error writing secondary frame to recording: {e}")
        ...
        if st['state'] == 'outside':
            ...
            if pos in ('A', 'B'):
                ...
                if st['entry_side'] and exit_side != st['entry_side']:
                    ...
                    # Check if target count is reached (only once)
    ...
    def run(self):
        ...
        try:
            ...
            while self.run_flag:
                ...
                if primary_frame is None and secondary_frame is None:
                    self.msleep(10)  # Sleep briefly if no frames available
                    continue
                
                # Process primary frame for display and detection
                primary_display_frame = None
                if primary_frame is not None:
                    ...
                    # NEW: Add counting status indicator
                    counting_status = "COUNTING: ACTIVE" if self.counting_active else "COUNTING: PAUSED"
                    counting_color = (0, 255, 0) if self.counting_active else (0, 165, 255)  # Green for active, orange for paused
                    cv2.putText(primary_display_frame, counting_status, (W-250, 35),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.7, counting_color, 2)
                ...
                # Process secondary frame for display (no annotations)
                secondary_display_frame = secondary_frame.copy() if secondary_frame is not None else None
                
                # Write frames to recordings
                if primary_display_frame is not None:
                    self.write_frame_to_recording(primary_display_frame, secondary_display_frame)
            ...
            # Ensure recordings are saved even if thread crashes
            try:
                if self.recording:
                    self.stop_recording()
            except:
                pass
            self.update_info_signal.emit(f"Thread error: {str(e)}")
        finally:
    ...
    # NEW: Start counting (resume)
    def start_counting(self):
        """Start or resume counting while keeping video and recording running"""
        self.counting_active = True
        self.counting_status_signal.emit(True)
        self.update_info_signal.emit("Counting resumed")
    
    # NEW: Stop counting (pause)
    def stop_counting(self):
        """Stop or pause counting while keeping video and recording running"""
        self.counting_active = False
        self.counting_status_signal.emit(False)
        self.update_info_signal.emit("Counting paused - video and recording continue")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.stop_btn = QPushButton("⏸️ Stop Counting")  # Changed from ⏹️ to ⏸️
    ...
    def start_counting(self):
        # NEW: Start video thread if not already running
        ...
        # NEW: Start/resume counting
        self.thread.start_counting()
        
        # Update UI state
        self.start_btn.setEnabled(False)
        self.stop_btn.setEnabled(True)
        
        self.update_info("Counting started/resumed")
    
    def stop_counting(self):
        # NEW: Stop/pause counting but keep video and recording running
        self.thread.stop_counting()
        
        # Update UI state
        self.start_btn.setEnabled(True)
        self.stop_btn.setEnabled(False)
        
        self.update_info("Counting paused - video and recording continue")
