...
class CementBagOutCounterApp(QMainWindow):
    def __init__(self):
        ...
        self.rtsp_urls_camera1 = {
            "Camera 1 - Default": "rtsp://192.168.1.101:8554/mystream",
            "Camera 1 - Custom": ""
        }
        
        self.rtsp_urls_camera2 = {
            "Camera 2 - Default": "rtsp://192.168.1.101:8554/mystream",
            "Camera 2 - Custom": ""
        }
    ...
    def init_ui(self):
        ...
        for i in range(2):
            ...
            camera_title.setStyleSheet("font-weight: bold; color: #3498db; font-size: 14px;")
    ...
    def reset_region_state(self, cam_idx):
        ...
        # Remove mouse event handlers for region drawing (but keep fullscreen toggle)
        ...
        if active:
            ...
            if self.video_streams[cam_idx]:
                ...
                if self.first_frame[cam_idx] is not None:
                    ...
                    # If we have saved points, show them
                    if self.region_points[cam_idx] and self.region_fixed[cam_idx]:
                        ...
                        self.show_colored_message(
                            "Success",
                            f"Camera {cam_idx+1} connected successfully! Previously saved counting region loaded.\n"
            ...
            # Connection failed
            self.camera_connected[cam_idx] = False
            
            if cam_idx == 0:
                self.camera1_connect_btn.setEnabled(True)
                self.camera1_connect_btn.setText("Connect Camera 1")
                self.camera1_status.setText("Status: Connection Failed")
                self.camera1_status.setStyleSheet("color: #e74c3c; font-weight: bold;")
                self.region1_btn.setEnabled(False)
            else:
                self.camera2_connect_btn.setEnabled(True)
                self.camera2_connect_btn.setText("Connect Camera 2")
                self.camera2_status.setText("Status: Connection Failed")
                self.camera2_status.setStyleSheet("color: #e74c3c; font-weight: bold;")
                self.region2_btn.setEnabled(False)
                
            self.show_colored_message(
                "Error",
                f"Failed to connect to Camera {cam_idx+1}. Please check the RTSP URL and try again.",
    ...
    def load_model(self):
        try:
            ...
            if os.path.exists(model_path):
                ...
                print(f"Model file not found at {model_path}, using default YOLOv8 model")
